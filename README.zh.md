...
L3 — 高级配置（< 2 小时）
    ...
    依赖: GitHub Actions + gitleaks + semgrep + serena + typescript-lsp + playwright
```
...
| **执行角色** | Agent（有前端时调用 `e2e-test`，支持 Playwright MCP 交互模式 / Playwright 脚本 / agent-browser 三种路径） |
...
└── reports/                    # 核验报告 + MCP Trace 记录（/verify-phase + MCP 产出）
    ...
    └── mcp-traces/              # MCP 浏览器交互 Trace（e2e-test Phase 1c 产出）
        ├── {phase}-index.md     # 每 Phase Trace 索引
...
| `agent-browser` | `e2e-test` Skill 内部调用（Playwright MCP 不可用时的 fallback） |
...
| [Unix Philosophy](https://en.wikipedia.org/wiki/Unix_philosophy) | Simplicity First 原则：做一件事并做好；最小惊讶原则 |
| [Principle of Least Astonishment](https://en.wikipedia.org/wiki/Principle_of_least_astonishment) | Surgical Changes 原则：变更不应超出读者预期 |
